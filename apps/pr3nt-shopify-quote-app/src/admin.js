...
function normalizeQuoteLines(body) {
  ...
  })).filter((line) => line.label || money(line.unit) > 0 || line.description);
}
...
function quoteLines(quote) {
  ...
  return [
    { label: 'Filament', description: '€0,12 per gram', qty: '', unit: '' },
    { label: 'Printtijd', description: '€0,35 per uur', qty: '', unit: '' },
    { label: 'Opstartkosten', description: 'Vaste startkosten', qty: '1', unit: '7,50' },
  ];
}
...
function renderPage(title, body) {
  return `<!doctype html><html lang="nl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)} · pr3nt Dashboard</title><style>
    :root{--bg:#f6f6f7;--card:#fff;--ink:#202223;--muted:#6d7175;--line:#e1e3e5;--green:#00d084;--dark:#111827;--radius:16px}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;line-height:1.5}a{color:#2c6ecb;text-decoration:none}.shell{max-width:1320px;margin:0 auto;padding:22px}.topbar{display:flex;justify-content:space-between;gap:16px;align-items:center;margin-bottom:20px}.brand{font-weight:950;font-size:24px;letter-spacing:-.04em;color:#101820}.muted{color:var(--muted)}.card{background:var(--card);border:1px solid var(--line);border-radius:var(--radius);box-shadow:0 1px 0 rgba(0,0,0,.04),0 10px 28px rgba(0,0,0,.05);padding:20px}.login-card{max-width:430px;margin:80px auto}.stack{display:grid;gap:12px}.three{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px}input,select,textarea{width:100%;border:1px solid #c9cccf;border-radius:10px;padding:10px 12px;font:inherit;background:#fff}textarea{min-height:100px}label span{display:block;font-weight:750;margin-bottom:5px}.button{border:0;border-radius:10px;background:var(--dark);color:#fff;padding:10px 14px;font-weight:800;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;gap:8px;min-height:42px}.button.green{background:var(--green);color:#082115}.button.ghost{background:#fff;color:var(--ink);border:1px solid #c9cccf}.button.icon{width:38px;min-height:38px;padding:0;border-radius:999px}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:18px}.stat strong{display:block;font-size:24px}.toolbar{display:flex;gap:10px;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:13px;border-bottom:1px solid var(--line);vertical-align:top}th{font-size:12px;color:var(--muted);text-transform:uppercase;letter-spacing:.08em}.badge{display:inline-flex;border-radius:999px;background:#f1f2f3;padding:5px 10px;font-size:12px;font-weight:800}.rush{background:#fff4e5;color:#8a4b00}.detail-grid{display:grid;grid-template-columns:1fr 390px;gap:18px}.file-link{display:block;margin:6px 0}.notice{padding:12px 14px;border-radius:14px;background:#ecfdf3;border:1px solid #bbf7d0;color:#166534;margin-bottom:14px}.message{padding:12px;border-radius:12px;background:#f6f6f7;border:1px solid var(--line)}.quote-tools{display:flex;justify-content:space-between;gap:12px;align-items:center;margin:14px 0}.quote-lines{display:grid;gap:10px}.quote-line{display:grid;grid-template-columns:1.15fr 1.25fr .55fr .65fr 40px;gap:8px;align-items:end;padding:12px;border:1px solid var(--line);border-radius:14px;background:#fafafa}.quote-line.is-empty{display:none}.total{display:flex;justify-content:space-between;align-items:center;padding-top:14px;border-top:1px solid var(--line);font-weight:950;font-size:20px}.total small{display:block;font-weight:600;font-size:12px;color:var(--muted)}@media(max-width:850px){.grid,.detail-grid,.three,.quote-line{grid-template-columns:1fr}.shell{padding:14px}table{font-size:14px}.button.icon{width:100%}}
  </style></head><body><main class="shell"><div class="topbar"><a class="brand" href="/admin">pr3nt Client Dashboard</a><span class="muted">Shopify app beheeromgeving</span></div>${body}</main><script>
    ...
    function refreshQuoteEditor(){var total=0;document.querySelectorAll('[data-quote-line]').forEach(function(row){var qty=row.querySelector('[name="lineQty"]');var unit=row.querySelector('[name="lineUnit"]');total+=parseMoney(qty&&qty.value?qty.value:1)*parseMoney(unit&&unit.value?unit.value:0)});var target=document.querySelector('[data-quote-total]');if(target)target.textContent='€ '+formatMoney(total)}
...
function renderQuoteLineInputs(quote) {
  ...
  const allLines = [...lines, ...Array.from({ length: Math.max(4, 12 - lines.length) }, () => ({ label: '', description: '', qty: '', unit: '' }))];
  return `<div class="quote-tools"><p class="muted" style="margin:0">Gebruik + om regels toe te voegen. Lege regels worden niet opgeslagen.</p><button class="button ghost" type="button" data-add-line>+ Regel toevoegen</button></div><div class="quote-lines">${allLines.map((line) => {
    const empty = !line.label && !line.description && !line.qty && !line.unit;
