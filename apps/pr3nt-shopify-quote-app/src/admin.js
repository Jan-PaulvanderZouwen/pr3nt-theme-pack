...
function normalizeQuoteLines(body) {
  ...
  })).filter((line) => line.label || money(line.unit) > 0);
}
...
function quoteLines(quote) {
  ...
  return [
    ...
    { label: 'Verpakkingsmateriaal', description: 'Veilig verpakt verzenden', qty: '1', unit: '3,50' },
    { label: 'Verzending', description: 'Alleen verzenden', qty: '1', unit: '6,55' },
  ];
}
...
function renderQuoteLineInputs(quote) {
  ...
  const allLines = [...lines, ...Array.from({ length: emptyRows }, () => ({ label: '', description: '', qty: '', unit: '' }))];
  return allLines.map((line) => `<div class="quote-line"><label><span>Regel</span><input name="lineLabel" form="quote-form" value="${escapeHtml(line.label || '')}" placeholder="Bijv. Filament"></label><label><span>Omschrijving</span><input name="lineDescription" form="quote-form" value="${escapeHtml(line.description || '')}" placeholder="Bijv. €0,12 per gram"></label><label><span>Aantal</span><input name="lineQty" form="quote-form" value="${escapeHtml(line.qty || '')}" placeholder="1"></label><label><span>Prijs</span><input name="lineUnit" form="quote-form" value="${escapeHtml(line.unit || '')}" placeholder="0,00"></label></div>`).join('');
}
