...
const archiveRetentionDays = Number(process.env.QUOTE_ARCHIVE_RETENTION_DAYS || 30);
...
function applyAutomaticStatus(quote) {
  if (quote.archivedAt || ['print_queue', 'ready_to_ship', 'shipped', 'delivered'].includes(quote.status)) return;
  if (quote.trackingCode) return void (quote.status = 'shipped');
  if (quote.paidAt || quote.status === 'paid') return void (quote.status = 'paid');
  if (quote.acceptedAt || quote.status === 'accepted') return void (quote.status = 'accepted');
  if (quoteTotal(quote) > 0) return void (quote.status = 'quote_sent');
  quote.status = quote.status || 'received';
}

function archiveDeleteDate(archivedAt = new Date().toISOString()) {
  const date = new Date(archivedAt);
  date.setDate(date.getDate() + archiveRetentionDays);
...
function quoteStoredFileNames(quote) {
  ...
  if (Array.isArray(quote.files)) {
    quote.files.forEach((file) => {
      if (file?.storedName) names.add(file.storedName);
    });
  }
  return [...names].filter(Boolean);
}

async function deleteQuoteFiles(quote) {
  await Promise.all(quoteStoredFileNames(quote).map((fileName) => unlink(path.join(uploadDir, fileName)).catch(() => {})));
}

async function purgeExpiredArchives(quotes) {
  const now = Date.now();
  const keep = [];
  let changed = false;
  for (const quote of quotes) {
    if (quote.archivedAt && new Date(quote.deleteAfter || archiveDeleteDate(quote.archivedAt)).getTime() <= now) {
      await deleteQuoteFiles(quote);
      changed = true;
    } else {
      keep.push(quote);
    }
  }
  return { quotes: keep, changed };
}
...
async function readQuotes() {
  ...
  const purged = await purgeExpiredArchives(quotes);
  if (purged.changed) await writeQuotes(purged.quotes);
  return purged.quotes;
}
...
async function notifyCustomerQuoteSent(quote) {
  if (!quote.email || quote.quoteSentAt || quoteTotal(quote) <= 0 || quote.archivedAt) return;
  const mailer = transporter();
  const from = process.env.MAIL_FROM || process.env.SMTP_USER;
  await mailer.sendMail({ from, to: quote.email, subject: 'Je offerte van pr3nt.nl staat klaar', html: quoteEmailHtml(quote) });
...
function renderPage(title, body) {
  return `<!doctype html><html lang="nl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)} · pr3nt Dashboard</title><style>
    :root{--bg:#f6faf7;--card:#fff;--ink:#103f31;--muted:#60756b;--line:#d7e4dc;--green:#087654;--dark:#103f31;--red:#d72c0d;--radius:22px}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;line-height:1.5}a{color:#087654;text-decoration:none}.shell{max-width:1320px;margin:0 auto;padding:22px}.topbar{display:flex;justify-content:space-between;gap:16px;align-items:center;margin-bottom:20px}.brand{font-weight:950;font-size:24px;letter-spacing:-.04em;color:#101820}.muted{color:var(--muted)}.card{background:var(--card);border:1px solid var(--line);border-radius:var(--radius);box-shadow:0 1px 0 rgba(0,0,0,.04),0 10px 28px rgba(0,0,0,.05);padding:20px}.portal-hero{display:grid;grid-template-columns:1.2fr .8fr;gap:20px;align-items:center;margin-bottom:18px;background:linear-gradient(135deg,#103f31,#087654);color:#fff;border:0}.portal-hero .muted{color:#d7eee1}.eyebrow{font-size:12px;text-transform:uppercase;letter-spacing:.12em;font-weight:900;color:#8fe0b4}.quick-links{display:grid;gap:10px}.quick-card{display:block;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.22);border-radius:16px;padding:14px;color:#fff;text-decoration:none}.quick-card strong{display:block;font-size:16px}.quick-card span{display:block;color:#d7eee1;font-size:13px;margin-top:3px}.login-card{max-width:430px;margin:80px auto}.stack{display:grid;gap:12px}.three{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px}.compact-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.info-card{border:1px solid var(--line);border-radius:14px;padding:14px;background:#f2f8f3}.info-card small{display:block;color:var(--muted);font-weight:800;text-transform:uppercase;letter-spacing:.05em}.info-card strong{display:block;margin-top:4px}input,select,textarea{width:100%;border:1px solid #c9cccf;border-radius:999px;padding:10px 12px;font:inherit;background:#fff}textarea{min-height:100px}label span{display:block;font-weight:750;margin-bottom:5px}.button{border:0;border-radius:999px;background:var(--dark);color:#fff;padding:10px 14px;font-weight:800;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;gap:8px;min-height:42px}.button.green{background:var(--green);color:#082115}.button.ghost{background:#fff;color:var(--ink);border:1px solid #c9cccf}.button.danger{background:#fff1f0;color:var(--red);border:1px solid #fed3d1}.button.icon{width:38px;min-height:38px;padding:0;border-radius:999px}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:18px}.stat strong{display:block;font-size:24px}.toolbar{display:flex;gap:10px;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:13px;border-bottom:1px solid var(--line);vertical-align:top}th{font-size:12px;color:var(--muted);text-transform:uppercase;letter-spacing:.08em}.badge{display:inline-flex;border-radius:999px;background:#f1f2f3;padding:5px 10px;font-size:12px;font-weight:800}.badge.green{background:#e9fbf2;color:#087443}.badge.red{background:#fff1f0;color:#d72c0d}.rush{background:#fff4e5;color:#8a4b00}.detail-grid{display:grid;grid-template-columns:1fr 390px;gap:18px}.file-link{display:block;margin:6px 0}.notice{padding:12px 14px;border-radius:14px;background:#ecfdf3;border:1px solid #bbf7d0;color:#166534;margin-bottom:14px}.message{padding:12px;border-radius:12px;background:#f6f6f7;border:1px solid var(--line)}.quote-tools{display:flex;justify-content:space-between;gap:12px;align-items:center;margin:14px 0}.quote-lines{display:grid;gap:10px}.quote-line{display:grid;grid-template-columns:1.15fr 1.25fr .55fr .65fr 40px;gap:8px;align-items:end;padding:12px;border:1px solid var(--line);border-radius:14px;background:#f2f8f3}.quote-line.is-empty{display:none}.total{display:flex;justify-content:space-between;align-items:center;padding-top:14px;border-top:1px solid var(--line);font-weight:950;font-size:20px}.total small{display:block;font-weight:600;font-size:12px;color:var(--muted)}details>summary{cursor:pointer;list-style:none;display:flex;justify-content:space-between;gap:12px;align-items:center}details>summary::-webkit-details-marker{display:none}.summary-title{font-size:18px;font-weight:900}.edit-pill{border:1px solid var(--line);border-radius:999px;padding:6px 10px;background:#fff;font-weight:800}.danger-zone{border-color:#fed3d1;background:#fffafa}input,select,textarea{border-radius:12px}@media(max-width:850px){.portal-hero{grid-template-columns:1fr}.grid,.detail-grid,.three,.quote-line,.compact-grid{grid-template-columns:1fr}.shell{padding:14px}table{font-size:14px}.button.icon{width:100%}}
  </style></head><body><main class="shell"><div class="topbar"><a class="brand" href="/admin">pr<span style="color:#087654">3</span>nt portaal</a><div class="toolbar"><a class="button ghost" href="/admin">Aanvragen</a><a class="button ghost" href="/admin/website">Website CMS</a><a class="button ghost" href="https://pr3nt.nl" target="_blank" rel="noreferrer">Website bekijken ↗</a></div></div>${body}</main><script>
...
function renderAdminList(quotes) {
  ...
  const archived = quotes.length - activeQuotes.length;
  const total = activeQuotes.length;
  const open = activeQuotes.filter((q) => !['delivered', 'shipped'].includes(q.status)).length;
  const quoteSent = activeQuotes.filter((q) => q.status === 'quote_sent').length;
  const paid = activeQuotes.filter((q) => q.status === 'paid').length;
  const rows = activeQuotes.map((quote) => `<tr><td><strong>${escapeHtml(quote.name)}</strong><br><span class="muted">${escapeHtml(quote.email)}</span></td><td>${escapeHtml(quote.material)} · ${escapeHtml(quote.color)}${quote.rush === 'Ja' ? '<br><span class="badge rush">Spoed</span>' : ''}</td><td><span class="badge ${quote.status === 'accepted' || quote.status === 'paid' ? 'green' : ''}">${escapeHtml(statusLabel(quote.status))}</span></td><td>${quoteTotal(quote) ? `€ ${fmt(quoteTotal(quote))}` : '-'}</td><td>${escapeHtml(new Date(quote.createdAt || Date.now()).toLocaleDateString('nl-NL'))}</td><td><a class="button ghost" href="/admin/quotes/${encodeURIComponent(quote.id)}">Openen</a></td></tr>`).join('');
  return renderPage('Aanvragen', `<section class="card portal-hero"><div><div class="eyebrow">PR3NT PORTAAL</div><h1 style="margin:8px 0 10px;color:#fff">Alles voor je printservice op één plek.</h1><p class="muted" style="margin:0">Beheer aanvragen, klantportalen en je website vanuit dezelfde omgeving.</p></div><div class="quick-links"><a class="quick-card" href="/admin/website"><strong>Website CMS →</strong><span>Teksten, SEO en media beheren</span></a><a class="quick-card" href="/admin/archive"><strong>Archief →</strong><span>Bekijk afgeronde aanvragen</span></a></div></section><div class="grid"><div class="card stat"><span class="muted">Totaal</span><strong>${total}</strong></div><div class="card stat"><span class="muted">Actief</span><strong>${open}</strong></div><div class="card stat"><span class="muted">Offerte verstuurd</span><strong>${quoteSent}</strong></div><div class="card stat"><span class="muted">Betaald</span><strong>${paid}</strong></div></div><section class="card"><div class="toolbar"><div><h1 style="margin:0">Klantenoverzicht</h1><p class="muted" style="margin:4px 0 0">Alle offerte-aanvragen en actieve projecten.</p></div></div><div class="table-wrap"><table><thead><tr><th>Klant</th><th>Print</th><th>Status</th><th>Offerte</th><th>Datum</th><th></th></tr></thead><tbody>${rows || '<tr><td colspan="6" class="muted">Nog geen aanvragen gevonden.</td></tr>'}</tbody></table></div><p class="muted" style="margin:18px 0 0;font-size:12px"><a href="/admin/archive">Archief bekijken</a>${archived ? ` · ${archived} aanvraag/aanvragen in archief` : ''}</p></section>`);
}

function renderArchiveList(quotes) {
  const archivedQuotes = quotes.filter((quote) => quote.archivedAt);
  const rows = archivedQuotes.map((quote) => `<tr><td><strong>${escapeHtml(quote.name || 'Gearchiveerde aanvraag')}</strong><br><span class="muted">${escapeHtml(quote.email || '')}</span></td><td>${escapeHtml(quote.id)}</td><td>${escapeHtml(new Date(quote.archivedAt).toLocaleDateString('nl-NL'))}</td><td>${escapeHtml(new Date(quote.deleteAfter || archiveDeleteDate(quote.archivedAt)).toLocaleDateString('nl-NL'))}</td></tr>`).join('');
  return renderPage('Archief', `<section class="card"><div class="toolbar"><div><h1 style="margin:0">Archief</h1><p class="muted">Gearchiveerde aanvragen worden na ${archiveRetentionDays} dagen automatisch definitief verwijderd, inclusief bestanden.</p></div><a class="button ghost" href="/admin">Terug</a></div><div class="table-wrap"><table><thead><tr><th>Klant</th><th>Aanvraag</th><th>Gearchiveerd</th><th>Verwijderen op</th></tr></thead><tbody>${rows || '<tr><td colspan="4" class="muted">Het archief is leeg.</td></tr>'}</tbody></table></div></section>`);
}
...
export function registerAdminRoutes(app) {
  ...
  app.get('/admin/archive', requireAdmin, async (_req, res) => {
    const quotes = await readQuotes();
    res.send(renderArchiveList(quotes));
  });
  ...
  app.post('/admin/quotes/:id/delete', requireAdmin, async (req, res) => {
    ...
    quote.deleteAfter = archiveDeleteDate(now);
    quote.previousStatus = quote.status;
    quote.status = 'archived';
    quote.portalToken = `archived-${randomUUID()}`;
    quote.messages = Array.isArray(quote.messages) ? quote.messages : [];
    quote.messages.push({ from: 'pr3nt', text: 'Aanvraag is verwijderd en 30 dagen gearchiveerd.', createdAt: now });
    await writeQuotes(quotes);
