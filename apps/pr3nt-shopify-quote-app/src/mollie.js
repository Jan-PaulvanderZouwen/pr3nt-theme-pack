...
function ensurePortalToken(quote) {
  if (!quote.portalToken) quote.portalToken = crypto.randomUUID();
...
async function createMolliePaymentLink(quote, amount) {
  ...
  const response = await fetch('https://api.mollie.com/v2/payment-links', {
    ...
    body: JSON.stringify({
      ...
      billingAddress: quote.email ? {
        email: quote.email,
        givenName: String(quote.name || '').split(/\s+/).filter(Boolean)[0] || quote.name || 'Klant',
        familyName: String(quote.name || '').split(/\s+/).filter(Boolean).slice(1).join(' ') || quote.name || 'pr3nt',
      } : undefined,
    }),
  });
...
function paymentMessageExists(quote, text) {
...
export function registerMollieRoutes(app) {
  app.use('/admin/quotes/:id', async (req, _res, next) => {
    ...
    try {
      ...
      if (!paymentMessageExists(quote, 'Mollie-betaallink aangemaakt')) {
        quote.messages.push({ from: 'pr3nt', text: 'Mollie-betaallink aangemaakt.', createdAt: quote.updatedAt });
      }
  ...
  app.post('/api/mollie/webhook', async (req, res) => {
    try {
      ...
      if (payment.status === 'paid') {
        ...
        quote.messages = Array.isArray(quote.messages) ? quote.messages : [];
        if (!paymentMessageExists(quote, 'Betaling ontvangen via Mollie')) {
