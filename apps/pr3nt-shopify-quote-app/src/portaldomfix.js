...
function priceBlockHtml(quote) {
  ...
  if (!total) {
    return `<div class="price-card"><span class="eyebrow">Prijsopgaaf</span><h2>Prijs wordt berekend</h2><p class="muted">We bekijken je bestand en berekenen de prijs. Verzending is automatisch inbegrepen.</p></div>`;
  }
  if (quote.paymentUrl && !alreadyFinished) {
  ...
  if (quote.paymentUrl && alreadyFinished) {
    return `<div class="price-card"><span class="eyebrow">Betaling</span><h2>€ ${fmt(total)}</h2><p class="muted">De betaling is verwerkt. Verzending is inbegrepen.</p></div>`;
  }
  if (quote.acceptedAt) {
...
function script(quote) {
  ...
  return `<script id="pr3nt-final-portal-js">
    (function(){
      ...
      var insertAfter = document.querySelector('.project-info-grid') || document.querySelector('section.grid.one-card') || document.querySelector('section.grid') || document.querySelector('.account-section');
      if (insertAfter && selfService) insertAfter.insertAdjacentHTML('afterend', selfService);
    })();
  </script>`;
}
