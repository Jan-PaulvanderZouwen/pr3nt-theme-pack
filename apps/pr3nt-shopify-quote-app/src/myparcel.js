...
function requireAdminRole(req, res, next) {
  ...
  return res.status(403).send(shell('work', user, `<div class="top"><div><span class="eyebrow">Geen toegang</span><h1>Pagina niet beschikbaar</h1><p class="muted">Deze pagina is alleen beschikbaar voor beheerders.</p></div></div><section class="card"><a class="button" href="/admin/work">Naar werkruimte</a></section>`));
}
...
function decorateClassicAdminHtml(req, html) {
  ...
  output = output.replace('<body><main class="shell">', `<body><main class="unified-app">${navHtml('admin', user)}<section class="unified-content"><div class="admin-classic-title"><div><span class="eyebrow">Beheer</span><h1>Admin orders</h1><p class="muted">Aanvragen, offertes, betalingen en klantcommunicatie.</p></div><a class="button light" href="/admin/work">Productie openen</a></div><div class="shell">`);
...
function calendarBlocks(quotes, settings, selectedDate) {
  ...
  return settings.printers.map((printer) => {
    ...
    return `<section class="printer-lane"><div class="printer-head">${e(printer.name)}</div><div class="printer-body">${hourLines}${html || '<div style="padding:14px;color:#667085">Geen ingeplande prints</div>'}</div></section>`;
  }).join('');
}

function workAgendaHtml(quotes, settings, selectedDate, user) {
  return shell('agenda', user, `<div class="top"><div><span class="eyebrow">Planning</span><h1>Printeragenda</h1><p class="muted">Beschikbaarheid per printer, gebaseerd op de ingeplande printuren.</p></div><form class="agenda-tools" method="get" action="/admin/work/agenda"><input type="date" name="date" value="${e(selectedDate)}"><button type="submit">Toon dag</button></form></div><section class="printer-grid">${calendarBlocks(quotes, settings, selectedDate)}</section>`);
}
...
function workOptionsHtml(settings, user, saved = false) {
  ...
  return shell('options', user, `<div class="top"><div><span class="eyebrow">Instellingen</span><h1>Productie-instellingen</h1><p class="muted">Printers, printernamen en agenda-synchronisatie.</p></div><a class="button light" href="/admin/users">Gebruikers beheren</a></div>${saved ? '<div class="card" style="margin-bottom:16px;background:#ecfdf3">Instellingen opgeslagen.</div>' : ''}<section class="grid2"><form class="card small-form" method="post" action="/admin/work/options"><h2>Printers</h2><label><span>Aantal printers</span><input name="printerCount" type="number" min="1" max="20" value="${e(settings.printerCount)}"></label><div class="settings-printers">${printerInputs}</div><h2>Agenda-synchronisatie</h2><p class="muted">Alle ingeplande prints worden gepubliceerd in een alleen-lezen ICS-feed.</p><label><span>Agenda-token</span><input name="calendarToken" value="${e(calendarToken)}"></label><label><span>ICS-feed</span><input readonly value="${e(feedUrl)}"></label>${settings.users.map((account) => `<input type="hidden" name="userName" value="${e(account.username)}"><input type="hidden" name="userDisplayName" value="${e(account.name || account.username)}"><input type="hidden" name="userRole" value="${e(account.role)}"><input type="hidden" name="userPasswordHash" value="${e(account.passwordHash)}">`).join('')}<button type="submit">Instellingen opslaan</button></form><div class="card"><h2>Planning</h2><p class="muted">Orders worden in de printeragenda geplaatst op basis van printer, starttijd en printduur.</p><p class="muted">De printduur wordt automatisch gevuld vanuit de offerte-regel Print-uren en kan per order worden aangepast.</p></div></section>`);
}
