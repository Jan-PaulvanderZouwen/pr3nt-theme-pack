...
function sessionUser(req) {
  ...
  if (payload && signature && safeEquals(signature, sign(payload))) {
    try { return JSON.parse(Buffer.from(payload, 'base64url').toString('utf8')); } catch {}
  }
  if (hasAdminAccess(req)) return { username: 'admin', role: 'admin', name: 'Admin' };
  if (hasLegacyWorkerAccess(req)) return { username: 'productie', role: 'worker', name: 'Productie' };
  return null;
}

function isAdmin(user) { return user?.role === 'admin'; }
function isWorker(user) { return user?.role === 'worker' || user?.role === 'admin'; }
...
async function writeSettingsFromBody(body = {}) {
  ...
  const usernames = Array.isArray(body.userName) ? body.userName : body.userName ? [body.userName] : [];
  const displayNames = Array.isArray(body.userDisplayName) ? body.userDisplayName : body.userDisplayName ? [body.userDisplayName] : [];
  const roles = Array.isArray(body.userRole) ? body.userRole : body.userRole ? [body.userRole] : [];
  const passwords = Array.isArray(body.userPassword) ? body.userPassword : body.userPassword ? [body.userPassword] : [];
  const existingHashes = Array.isArray(body.userPasswordHash) ? body.userPasswordHash : body.userPasswordHash ? [body.userPasswordHash] : [];
  ...
  const users = usernames.map((username, index) => {
    const cleanUsername = clean(username, 80);
    const password = String(passwords[index] || '');
    const existing = settings.users.find((user) => user.username === cleanUsername) || {};
    return {
      username: cleanUsername,
      name: clean(displayNames[index] || cleanUsername, 120),
      role: roles[index] === 'admin' ? 'admin' : 'worker',
      passwordHash: password ? passwordHash(password) : clean(existingHashes[index] || existing.passwordHash || '', 300),
    };
...
function hasLegacyWorkerAccess(req) { const workerKey = process.env.PRINT_WORKER_KEY || ''; const key = req.cookies?.pr3nt_worker_key || req.get('x-worker-key') || ''; return Boolean(workerKey && safeEquals(key, workerKey)); }
function requireInternal(req, res, next) { const user = sessionUser(req); if (isWorker(user)) { req.internalUser = user; return next(); } return res.status(401).send(loginHtml()); }
function requireAdminRole(req, res, next) { const user = sessionUser(req); if (isAdmin(user)) { req.internalUser = user; return next(); } return res.status(403).send(shell('work', user, `<div class="top"><div><span class="eyebrow">Geen toegang</span><h1>Niet beschikbaar</h1><p class="muted">Je account heeft geen toegang tot admin-instellingen.</p></div></div><section class="card"><a class="button" href="/admin/work">Terug naar werkruimte</a></section>`)); }
...
function firstShipmentId(result = {}) {
  ...
  if (Array.isArray(ids)) { const first = ids[0]; if (typeof first === 'number' || typeof first === 'string') return String(first); return String(first?.id || first?.shipment_id || first?.shipmentId || ''); }
...
export async function createMissingMyParcelLabels() {
  ...
  for (const quote of quotes) {
    ...
    try {
      ...
      if (result.skipped) { quote.myParcelStatus = 'skipped'; quote.myParcelMessage = result.reason; results.push({ id: quote.id, skipped: true, reason: result.reason }); }
      else { quote.myParcelStatus = 'created'; quote.myParcelShipmentId = result.shipmentId; quote.myParcelLabelUrl = result.labelUrl; quote.myParcelCreatedAt = now; quote.messages = Array.isArray(quote.messages) ? quote.messages : []; quote.messages.push({ from: 'pr3nt', text: `MyParcel-verzendlabel klaargezet${result.shipmentId ? `: ${result.shipmentId}` : ''}.`, createdAt: now }); results.push({ id: quote.id, shipmentId: result.shipmentId, labelUrl: result.labelUrl }); }
      changed = true;
    } catch (error) { quote.myParcelStatus = 'error'; quote.myParcelError = error.message; quote.myParcelTriedAt = now; results.push({ id: quote.id, error: error.message }); changed = true; }
  }
...
function estimatedPrintHours(quote = {}) {
  ...
  if (line) { const qty = money(line.qty || 0); if (qty > 0) return qty; const unit = money(line.unit || 0); const total = money(line.total || 0); if (unit > 0 && total > 0) return total / unit; }
  return 1;
}
function addHoursLocal(datetimeLocal, hours) { if (!datetimeLocal) return ''; const date = new Date(datetimeLocal); if (Number.isNaN(date.getTime())) return ''; date.setMinutes(date.getMinutes() + Math.max(0.25, hours) * 60); return toLocalInputValue(date); }
...
function navHtml(active = '', user = null) {
  const nav = isAdmin(user)
    ? [['admin', '/admin?classic=1', 'Admin orders'], ['work', '/admin/work', 'Werkruimte'], ['options', '/admin/work/options', 'Opties'], ['agenda', '/admin/work/agenda', 'Agenda'], ['stats', '/admin/work/stats', 'Statistieken']]
...
function decorateClassicAdminMiddleware(req, res, next) {
  if (req.method !== 'GET' || !req.path.startsWith('/admin') || req.path.startsWith('/admin/work')) return next();
...
function sameDay(value = '', selectedDate = '') { return Boolean(value && String(value).slice(0, 10) === selectedDate); }
function calendarBlocks(quotes, settings, selectedDate) {
  ...
  return settings.printers.map((printer) => {
    ...
    const html = printerBlocks.map((quote, index) => { const startMinutes = minutesFromStartOfDay(quote.scheduledStartAt); const hours = money(quote.estimatedPrintHours || estimatedPrintHours(quote)); const top = Math.max(0, Math.min(700, (startMinutes / 1440) * 720)); const height = Math.max(36, Math.min(720 - top, (Math.max(0.25, hours) * 60 / 1440) * 720)); return `<div class="print-block ${index % 2 ? 'block-soft' : ''}" style="top:${top}px;height:${height}px"><strong>${e(quote.name || 'Order')}</strong><span>${formatDateTime(quote.scheduledStartAt)} · ${fmt(hours)} uur</span><span>${e(quote.material || '-')} · ${e(quote.color || '-')}</span><span>${e(quote.id)}</span></div>`; }).join('');
...
function workAgendaHtml(quotes, settings, selectedDate, user) { return shell('agenda', user, `<div class="top"><div><span class="eyebrow">Printerplanning</span><h1>Blok-agenda</h1><p class="muted">Elke printer heeft een eigen baan. Orders blokkeren automatisch het aantal gequote printuren.</p></div><form class="agenda-tools" method="get" action="/admin/work/agenda"><input type="date" name="date" value="${e(selectedDate)}"><button type="submit">Toon dag</button></form></div><section class="printer-grid">${calendarBlocks(quotes, settings, selectedDate)}</section>`); }
...
function workOptionsHtml(settings, user, saved = false) {
  ...
  const userRows = [...settings.users, { username: '', name: '', role: 'worker', passwordHash: '' }].map((account) => `<div class="user-row"><label><span>Gebruikersnaam</span><input name="userName" value="${e(account.username)}"></label><label><span>Naam</span><input name="userDisplayName" value="${e(account.name || account.username)}"></label><label><span>Rol</span><select name="userRole"><option value="worker" ${account.role !== 'admin' ? 'selected' : ''}>Medewerker</option><option value="admin" ${account.role === 'admin' ? 'selected' : ''}>Admin</option></select></label><label><span>${account.username ? 'Nieuw wachtwoord' : 'Wachtwoord'}</span><input name="userPassword" type="password" autocomplete="new-password" placeholder="${account.username ? 'Leeg laten = behouden' : 'Nieuw account'}"><input type="hidden" name="userPasswordHash" value="${e(account.passwordHash)}"></label></div>`).join('');
  return shell('options', user, `<div class="top"><div><span class="eyebrow">Instellingen</span><h1>Opties</h1><p class="muted">Beheer printers, agenda-synchronisatie en interne gebruikers.</p></div></div>${saved ? '<div class="card" style="margin-bottom:16px;background:#ecfdf3">Instellingen opgeslagen.</div>' : ''}<section class="grid2"><form class="card small-form" method="post" action="/admin/work/options"><h2>Printers</h2><label><span>Aantal printers</span><input name="printerCount" type="number" min="1" max="20" value="${e(settings.printerCount)}"></label><div class="settings-printers">${printerInputs}</div><h2>Agenda-plugin</h2><p class="muted">Gebruik deze ICS-link als abonnement in Apple Agenda, Google Agenda of Outlook. Houd de token privé.</p><label><span>Agenda-token</span><input name="calendarToken" value="${e(calendarToken)}"></label><input readonly value="${e(feedUrl)}"><h2>Gebruikers</h2><p class="muted">Medewerkers zien alleen hun werkruimte en agenda. Admins zien ook admin-orders en opties.</p>${userRows}<button type="submit">Opslaan</button></form><div class="card"><h2>Toegang</h2><p class="muted">Een medewerker-login toont geen adminomgeving meer. De zijbalk laat alleen zien waar dat account toegang tot heeft.</p><p class="muted">Je eigen admin-login kan via gebruikersnaam <strong>admin</strong> en het huidige admin-wachtwoord, of via de bestaande admin-sleutel.</p></div></section>`);
}

function workStatsHtml(quotes, settings, user) { const active = quotes.filter((q) => !q.archivedAt); const paid = active.filter((q) => q.paidAt || q.status === 'paid'); const planned = active.filter((q) => q.scheduledStartAt); const productionTotal = paid.reduce((sum, q) => sum + money(q.productionAmount || q.productionOfferAmount || q.productionPrice || 0), 0); const plannedHours = planned.reduce((sum, q) => sum + money(q.estimatedPrintHours || estimatedPrintHours(q)), 0); return shell('stats', user, `<div class="top"><div><span class="eyebrow">Inzicht</span><h1>Statistieken</h1><p class="muted">Basisoverzicht voor productie en orders.</p></div></div><section class="cards"><div class="card stat"><span class="muted">Actieve orders</span><strong>${active.length}</strong></div><div class="card stat"><span class="muted">Betaald</span><strong>${paid.length}</strong></div><div class="card stat"><span class="muted">Geplande uren</span><strong>${fmt(plannedHours)}</strong></div><div class="card stat"><span class="muted">Printers</span><strong>${settings.printerCount}</strong></div></section><section class="cards"><div class="card stat"><span class="muted">Ingepland</span><strong>${planned.length}</strong></div><div class="card stat"><span class="muted">Productie totaal</span><strong>€ ${fmt(productionTotal)}</strong></div><div class="card stat"><span class="muted">Labels klaar</span><strong>${active.filter((q) => q.myParcelShipmentId).length}</strong></div><div class="card stat"><span class="muted">Spoed</span><strong>${active.filter((q) => q.rush === 'Ja').length}</strong></div></section>`); }
...
function calendarIcs(quotes, settings) { const events = eligibleOrders(quotes).filter((q) => q.scheduledStartAt && q.scheduledEndAt).map((quote) => { const printer = settings.printers.find((p) => p.id === quote.assignedPrinter || p.name === quote.assignedPrinter); const title = `Pr3nt print: ${quote.name || quote.id}`; const description = [`Order: ${quote.id}`, `Printer: ${printer?.name || quote.assignedPrinter || '-'}`, `Printuren: ${fmt(quote.estimatedPrintHours || estimatedPrintHours(quote))}`, `Materiaal: ${quote.material || '-'}`, `Kleur: ${quote.color || '-'}`].join('\n'); return ['BEGIN:VEVENT', `UID:${icsEscape(quote.id)}@pr3nt.nl`, `DTSTAMP:${icsDate(new Date().toISOString())}`, `DTSTART:${icsDate(quote.scheduledStartAt)}`, `DTEND:${icsDate(quote.scheduledEndAt)}`, `SUMMARY:${icsEscape(title)}`, `DESCRIPTION:${icsEscape(description)}`, 'END:VEVENT'].join('\r\n'); }).join('\r\n'); return ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Pr3nt//Printerplanning//NL', 'CALSCALE:GREGORIAN', 'METHOD:PUBLISH', 'X-WR-CALNAME:Pr3nt printerplanning', events, 'END:VCALENDAR'].filter(Boolean).join('\r\n'); }
...
export function registerMyParcelRoutes(app) {
  ...
  app.get('/admin', (req, res, next) => { if (req.query?.classic === '1') return next(); const user = sessionUser(req); if (isAdmin(user) || hasAdminAccess(req)) return res.redirect('/admin?classic=1'); return res.redirect('/admin/work'); });
  ...
  app.post('/admin/work/login', async (req, res) => {
    const username = req.body?.username || req.body?.workerKey || '';
    const password = req.body?.password || req.body?.workerKey || '';
  ...
  app.post('/admin/work/logout', (req, res) => { res.clearCookie?.(sessionCookie); res.clearCookie?.('pr3nt_worker_key'); res.redirect('/admin/work'); });

  app.get('/admin/work', requireInternal, async (req, res) => { const [quotes, settings] = await Promise.all([readQuotes(), readSettings()]); res.send(workOrdersHtml(quotes, settings, req.internalUser)); });
  app.get('/admin/work/options', requireAdminRole, async (req, res) => { const settings = await readSettings(); res.send(workOptionsHtml(settings, req.internalUser, req.query.saved === '1')); });
  app.post('/admin/work/options', requireAdminRole, async (req, res) => { await writeSettingsFromBody(req.body); res.redirect('/admin/work/options?saved=1'); });
  app.get('/admin/work/agenda', requireInternal, async (req, res) => { const [quotes, settings] = await Promise.all([readQuotes(), readSettings()]); const selectedDate = clean(req.query.date || new Date().toISOString().slice(0, 10), 10); res.send(workAgendaHtml(quotes, settings, selectedDate, req.internalUser)); });
  app.get('/admin/work/stats', requireAdminRole, async (req, res) => { const [quotes, settings] = await Promise.all([readQuotes(), readSettings()]); res.send(workStatsHtml(quotes, settings, req.internalUser)); });
  ...
  app.post('/admin/quotes/:id/myparcel', async (req, res) => {
    ...
    try {
      const result = await createMyParcelShipmentForQuote(quote);
      if (result.skipped) { quote.myParcelStatus = 'skipped'; quote.myParcelMessage = result.reason; }
      else { quote.myParcelStatus = 'created'; quote.myParcelShipmentId = result.shipmentId; quote.myParcelLabelUrl = result.labelUrl; quote.myParcelCreatedAt = now; }
