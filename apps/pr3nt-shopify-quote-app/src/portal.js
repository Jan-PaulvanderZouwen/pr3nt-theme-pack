...
function trackingCard(quote) {
  if (!quote.trackingCode) {
    return `<div class="tracking-card muted-card"><div class="tracking-icon">📦</div><div><h2>Verzending</h2><p class="muted">Zodra je print is verzonden, verschijnt hier de track & trace.</p></div></div>`;
  }
  const code = e(quote.trackingCode);
  return `<div class="tracking-card"><div class="tracking-icon">🚚</div><div><span class="eyebrow">Onderweg</span><h2>Je pakket volgen</h2><p class="muted">Gebruik onderstaande code bij de vervoerder.</p><div class="tracking-code">${code}</div></div></div>`;
}
...
function renderPortal(quote, allQuotes, saved = '') {
  ...
  return renderPage(`Aanvraag ${quote.id}`, `${saved ? `<div class="card" style="margin-bottom:18px"><strong>${e(saved)}</strong></div>` : ''}<section class="hero"><div class="card"><span class="badge">${e(label)}</span><h1>Welkom terug, ${e((quote.name || '').split(' ')[0] || 'maker')}</h1><p class="muted">Hier volg je je offerte, 3D-model, berichten en verzending.</p><div class="segmented">${segments}</div><div class="kv"><div class="muted">Aanvraag</div><div>${e(quote.id)}</div><div class="muted">Materiaal</div><div>${e(quote.material)}</div><div class="muted">Kleur</div><div>${e(quote.color)}</div><div class="muted">Spoed</div><div>${e(quote.rush || 'Nee')}</div></div></div><aside class="card"><h2>Offerte</h2>${quoteTable(quote)}</aside></section>${allProjectLinks(quote, allQuotes)}<section class="grid"><div class="card"><h2>3D model preview</h2><div id="viewer"><p class="muted">3D model wordt geladen...</p></div><div class="action-row">${file ? `<a class="btn btn-light" href="${e(file.url)}">Bestand downloaden</a>` : ''}<label class="btn btn-dark" for="upload-modal">Extra bestand uploaden</label></div>${uploadModal(quote)}</div><div class="card"><h2>Status</h2><div class="timeline">${timeline}</div></div></section><section class="grid"><div class="card"><h2>Berichten</h2>${renderMessages(quote)}</div><div class="card">${trackingCard(quote)}</div></section>`, file);
}
