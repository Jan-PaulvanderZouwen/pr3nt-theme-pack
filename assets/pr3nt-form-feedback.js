(function(){
  ...
  function requestType(form){
    var input = form.querySelector('[data-p-request-type-input]');
    return input ? input.value : '3d_file';
  }
  ...
  function filesAreAllowed(form){
    var input = form.querySelector('[data-p-file]');
    ...
    return files.every(function(file){
      ...
      return ext ? allowed.indexOf(ext) !== -1 : requestType(form) === 'no_model';
    });
  }
  ...
  function stepOneOk(form){
    ...
    if (requestType(form) === 'no_model') {
      return (files.length > 0 && filesAreAllowed(form)) || desc.length >= 6;
    }
    return files.length > 0 && filesAreAllowed(form);
  }

  function stepTwoOk(form){
    return value(form.querySelector('[data-p-material-input]')).length >= 2 && value(form.querySelector('[data-p-color]')).length >= 2;
  }

  function stepThreeOk(form){
    return value(form.querySelector('[data-p-print-size-input]')).length >= 2;
  }

  function contactOk(form){
    return value(form.querySelector('[data-p-name]')).length >= 2 &&
      validEmail(form.querySelector('[data-p-email]')) &&
      validPhone(form.querySelector('[data-p-phone]')) &&
      value(form.querySelector('[data-p-shipping-country]')).length >= 2 &&
      value(form.querySelector('[data-p-shipping-postal]')).length >= 4 &&
      value(form.querySelector('[data-p-shipping-house]')).length >= 1 &&
      value(form.querySelector('[data-p-shipping-city]')).length >= 2;
  }

  function stepOk(form, step){
    if (step === 1) return stepOneOk(form);
    if (step === 2) return stepTwoOk(form);
    if (step === 3) return stepThreeOk(form);
    return contactOk(form);
  }

  function formOk(form){
    return stepOneOk(form) && stepTwoOk(form) && stepThreeOk(form) && contactOk(form);
  }
  ...
  function currentStep(form){
    ...
    return visible ? Number(visible.getAttribute('data-p-step')) || 1 : Number(form.dataset.pr3ntFeedbackStep || 1);
  }

  function canReach(form, target){
    var current = currentStep(form);
    if (target <= current) return true;
    for (var i = 1; i < target; i += 1) {
      if (!stepOk(form, i)) return false;
    }
    return true;
  }
  ...
  function updateHiddenTabs(form){
    form.querySelectorAll('[data-p-tab]').forEach(function(tab){
      ...
      tab.classList.toggle('is-complete', tabStep < currentStep(form) || stepOk(form, tabStep));
  ...
  function ensureProgress(form){
    ...
    progress.innerHTML = [
      ...
      '<button type="button" class="p-form-progress-step" data-feedback-step="3"><span class="p-progress-dot">3</span><span>Formaat</span></button>',
      '<button type="button" class="p-form-progress-step" data-feedback-step="4"><span class="p-progress-dot">4</span><span>Gegevens</span></button>'
    ].join('');
    ...
    progress.querySelectorAll('[data-feedback-step]').forEach(function(button){
      button.addEventListener('click', function(event){
        ...
        setStep(form, Number(button.getAttribute('data-feedback-step')));
      });
    });
  }
  ...
  function updateProgress(form){
    ...
    progress.style.setProperty('--p-progress-ratio', String((step - 1) / 3));
    progress.querySelectorAll('[data-feedback-step]').forEach(function(button){
      var buttonStep = Number(button.getAttribute('data-feedback-step'));
      var reachable = canReach(form, buttonStep);
      button.classList.toggle('is-active', buttonStep === step);
      button.classList.toggle('is-done', buttonStep < step || stepOk(form, buttonStep));
  ...
  function updateHeader(form){
    var titles = {1:'Bestand of idee',2:'Materiaal & kleur',3:'Formaat & snelheid',4:'Gegevens'};
  ...
  function paymentLink(form){
    var input = form.querySelector('[data-p-payment-link-input]');
    return value(input);
  }
  ...
  function updateUi(form){
    ...
    if (submit) {
      submit.disabled = !formOk(form);
      submit.textContent = paymentLink(form) ? 'Bestand uploaden & afrekenen' : 'Offerte aanvragen';
    }
  }
  ...
  async function submitForm(form){
    ...
    try {
      ...
      var intakeNote = [
        '--- Aanvraagtype ---',
        requestType(form) === 'no_model' ? 'Nog geen 3D-bestand' : '3D-bestand aanwezig',
        value(form.querySelector('[data-p-description]')) ? '\n--- Omschrijving / referentie ---\n' + value(form.querySelector('[data-p-description]')) : '',
        '\n--- Printkeuze ---',
        'Materiaal: ' + value(form.querySelector('[data-p-material-input]')),
        'Kleur: ' + value(form.querySelector('[data-p-color]')),
        'Oppervlak: ' + value(form.querySelector('[data-p-print-size-input]')),
        'Richtprijs: ' + value(form.querySelector('[data-p-print-price-input]')),
        paymentLink(form) ? 'Betaallink: ' + paymentLink(form) : '',
        '\n--- Verzendadres ---',
        'Land: ' + value(form.querySelector('[data-p-shipping-country]')),
        'Postcode: ' + value(form.querySelector('[data-p-shipping-postal]')),
        'Huisnummer: ' + value(form.querySelector('[data-p-shipping-house]')),
        'Plaats: ' + value(form.querySelector('[data-p-shipping-city]'))
      ].filter(Boolean).join('\n');
