  ...
  function initForms(scope){
    ...
    forms.forEach(function(form){
      ...
      function updateSummary(){
        ...
        summary.innerHTML='<strong>'+fileName+'</strong><br><span>'+materialInput.value+' · '+(colorInput.value||'kleur niet ingevuld')+rushText+'</span>';
        if(fileNameInput) fileNameInput.value=f?f.name:'';
      }
      function update(){ if(nextButton)nextButton.disabled=!stepOneOk(); if(submitButton)submitButton.disabled=!allOk(); updateSummary(); }
      function setStep(n){ if(n===2&&!stepOneOk())return; step=n; if(stepOne)stepOne.hidden=step!==1; if(stepTwo)stepTwo.hidden=step!==2; tabs.forEach(function(b){b.classList.toggle('is-active',Number(b.dataset.pTab)===step)}); update(); }
      if(fileInput) fileInput.addEventListener('change',function(){ var f=fileInput.files[0]; if(!f){fileLabel.textContent='Sleep je 3D-bestand hierheen'; fileError.classList.remove('is-visible'); update(); return;} if(!validFile(f)){fileInput.value=''; fileLabel.textContent='Sleep je 3D-bestand hierheen'; fileError.classList.add('is-visible'); update(); return;} fileLabel.textContent=f.name; fileError.classList.remove('is-visible'); update(); });
      materialButtons.forEach(function(btn){btn.addEventListener('click',function(){materialButtons.forEach(function(i){i.classList.remove('is-active')}); btn.classList.add('is-active'); materialInput.value=btn.dataset.pMaterial; update();})});
      if(rushButton) rushButton.addEventListener('click',function(){var active=rushButton.classList.toggle('is-active'); rushInput.value=active?'Ja':'Nee'; var t=rushButton.querySelector('[data-p-rush-text]'); if(t)t.textContent=active?'+ €4,50 · 2 dagen verzending':'Standaard · 3 dagen verzending'; update();});
      ...
      form.addEventListener('submit',function(e){ if(!allOk()){e.preventDefault(); update();} });
      update();
    });
  }
